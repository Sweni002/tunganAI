...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (matricule, mot_de_passe) => {
    ...
    if (!data.role_conflict && !data.conflict && data.user) {
      setUser({ ...data.user, role: data.user.role });
    } else {
      setUser(data); // cas multi-rôle, traité ensuite dans Login.jsx
    }
  ...
  return (
    <AuthContext.Provider
      value={{
        user, loading, isLoggingOut, setUser,   // 🔹 ajouter cette ligne
