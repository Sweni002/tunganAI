...
useEffect(() => {
  ...
  setNom(record.nom || "") 
  setsigle(record.sigle || "")
  setAddresse(record.addresse || "");
setCode(record.code_service || "")

    if (record.logo) {
    setPreview(`data:image/png;base64,${record.logo}`);
  } else {
    setPreview(null);
  }
...
setMsg("")
  ...
  try {
       const response = await fetch(`${API_URL}/api/services/${record.idserv}`, {
      method: "PUT",
