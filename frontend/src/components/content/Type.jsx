...
const filteredservices = types.filter(p => {
  const lower = searchText.toLowerCase();

  // Vérifie correspondance texte
  const matchesSearch =
    p.nomtype.toLowerCase().includes(lower) || // Si aucune division sélectionnée, on affiche tout
    p.abbreviation.toLowerCase().includes(lower);  
...
useEffect(() => {
  ...
  const goAjout=()=>{
    navigate("/global/ajout_type")
  }


useEffect(() => {
    setLoading(true);
    fetchWithAuth(`${API_URL}/api/types/`)
      .then((data) => {
...
const handleConfirmDelete = () => {
    ...
    fetchWithAuth(`${API_URL}/api/types/${recordToDelete.idtype}`, {
      method: 'DELETE',
    })
      .then(() => {
        setSnackMessage("Type supprimé avec succès");
        setSnackError(false);
        setOpenSnack(true);
   settypes((prev) => prev.filter((p) => p.idtype !== recordToDelete.idtype));
      })
      .catch((err) => {
...
const columns = [
      ...
      >
        <Tooltip title="Modifier">
          <div
            ...
            onClick={() =>
              navigate("/global/modifier_type", { state: { record } })
            }
          >
            <IconButton
              aria-label="more"
  ...
  scroll={{ y: 540 }}
            ...
            columns={columns}
           dataSource={filteredservices.map(p => ({ ...p, key: p.idtype }))}
...
>
 ...
 <label htmlFor="id
     " style={{fontSize : "0.8rem" ,color :"#676767"}}>Voulez-vous vraiment supprimer ce type d'absence ?
