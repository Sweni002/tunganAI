...
import styles from "./ajout_div.module.css";
...
useEffect(() => {
  ...
  if (record) {
  console.log("donnee recu : " ,record)  
  setNom(record.nomdivision || "") 
...
formData.append("idserv", selectedService);
  ...
  try {
    const response = await fetch(`${API_URL}/api/divisions/${record.iddiv}`, {
      method: "PUT",
...
const action = (
  <>
    <Button color="primary" size="medium" onClick={() => {
        setOpenSnack(false);
        navigate("/global/responsable"); // navigation seulement ici
      }}
      sx={{p : 1 ,fontSize : 17}}>
       Voir
    </Button>
          ...
          <div className={styles.form}>
            <div className={styles.inputM}>
              <label htmlFor="matricule">
                Nom du division <span style={{ color: "red" }}>*</span>
              </label>
              <TextField
                placeholder="Entrez le nom du division"
                variant="standard"
                fullWidth
                value={nom}
                onChange={(e) => {
                ...
                error={!!errors.nom}
                helperText={errors.nom ? "Le nom du divison est requis." : ""}
                sx={{
                  mt: 1,
                  mb: 2,
                  width: "100%",
