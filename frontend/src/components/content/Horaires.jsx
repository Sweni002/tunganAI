...
const Horaires = () => {
  ...
  const goAjout = () => {
    navigate("/global/ajout_horaire");
  };

  useEffect(() => {
    setLoading(true);
    setLoadingPage(true);
    fetchWithAuth(`${API_URL}/api/horaires/`)
      .then((data) => {
  ...
  const handleConfirmDelete = () => {
    ...
    fetchWithAuth(`${API_URL}/api/services/${recordToDelete.idserv}/`, {
      ...
      .then(() => {
        ...
        setOpenSnack(true);
        setServices((prev) =>
          prev.filter((p) => p.idserv !== recordToDelete.idserv),
        );
      })
      .catch((err) => {
        ...
        >
          <Tooltip title="Modifier">
            <div
              ...
              onClick={() =>
                navigate("/global/modifier_service", { state: { record } })
              }
            >
              <IconButton
                aria-label="more"
      ...
      <div className={styles.cardTab}>
        ...
        <div className={`${styles.tableau} ${styles.shadowedTable}`}>
          <Table
            ...
            columns={columns}
            dataSource={filteredservices.map((p) => ({ ...p, key: p.idserv }))}
