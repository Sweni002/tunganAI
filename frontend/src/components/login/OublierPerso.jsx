...
const handleSubmit = async () => {
  ...
  try {
    const res = await fetch("https://192.168.43.10:5000/api/auth/send-reset-code-personnel", {
      method: "POST",
    ...
    if (!res.ok) {
      ...
      navigate("/accepte_perso", { state: { email } }); // <- on transmet l'email
    }
  } catch (err) {
    setError(true);
