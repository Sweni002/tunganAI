...
const AcceptePerso= () => {
 ...
 const [resendLoading, setResendLoading] = useState(false);
    ...
    useEffect(() => {
    if (!email) {
      navigate("/personnel"); // redirige vers /login
    }
  }, [email, navigate]);
...
const handleSubmit = async () => {
  ...
  try {
   const response = await fetch("https://192.168.0.115:5000/api/auth/verify-reset-code-personnel", {
      method: "POST",
    ...
    if (response.ok) {
      setError(false);
      setErrorMsg("");
          navigate("/mdp_perso", { state: { email ,code} });
    } else {
      setError(true);
...
console.log(email)
    try {
      const response = await fetch(`https://192.168.0.115:5000/api/auth/send-reset-code-personnel`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
